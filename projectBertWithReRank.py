...
class CrossAttentionReranker(nn.Module):
    ...
    def forward(self, query_ids: torch.Tensor, query_mask: torch.Tensor,
        ...
        # Concatenate query and passage with [SEP] token
        sep_token = torch.full((query_ids.size(0), 1), self.bert.config.sep_token_id,
...
class EnhancedHardNegativeMiner:
    def __init__(self, retriever: DenseRetrieverBERT, passage_embeddings: torch.Tensor,
                 passage_ids: List[int], num_negatives: int = 5):
    ...
    @torch.no_grad()
    def mine_hard_negatives(self, query_embeddings: torch.Tensor,
                            positive_ids_list: List[Set[int]]) -> List[List[int]]:
        # Compute similarity scores
        similarity = torch.matmul(query_embeddings, self.passage_embeddings.t())

        # Create mask to exclude all positive passages for each query
        positive_mask = torch.zeros_like(similarity, dtype=torch.bool)
        for i, pos_ids in enumerate(positive_ids_list):
            for pos_id in pos_ids:
                if pos_id in self.passage_ids:
                    positive_mask[i, self.passage_ids.index(pos_id)] = True

        # Set positive scores to -inf to exclude them
        similarity = similarity.masked_fill(positive_mask, float('-inf'))

        # Get top-k hard negatives
        _, hard_negative_indices = similarity.topk(k=self.num_negatives, dim=1)
        hard_negative_ids = [[self.passage_ids[idx] for idx in indices]
                             for indices in hard_negative_indices]
...
def train_enhanced_retriever(
    ...
    hard_negative_miner = EnhancedHardNegativeMiner(retriever, passage_embeddings, passage_ids)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # Second stage: Train reranker with hard negatives
            with torch.no_grad():
                hard_negative_ids = hard_negative_miner.mine_hard_negatives(
                    query_emb.detach(),
                    [set(ids.tolist()) for ids in batch['positive_ids']]
                )
        ...
        avg_retriever_loss = total_retriever_loss / len(train_dataloader)
        avg_reranker_loss = total_reranker_loss / len(train_dataloader)
...
def load_query_documents():
    ...
    for query in queries:
        ...
        doc_count = positive_docs[q_num].count()
...
def main():
    ...
    # Initialize passage embeddings for hard negative mining
    passage_embeddings = torch.randn(100, 768).to(device)
    passage_ids = list(range(100))
    ...
    # Save models
    torch.save(trained_retriever.state_dict(), "multi_positive_retriever.pt")
    torch.save(trained_reranker.state_dict(), "multi_positive_reranker.pt")
